# Audio Normalization Script for Windows
...
# Configuration
$TargetLUFS = "-14.0"
$LimiterThreshold = "-1.0"  # 1 dB headroom to prevent clipping
$LogFile = "normalization_log.txt"
...
function Normalize-File {
    ...
    $process = Start-Process -FilePath $FFMPEGPath -ArgumentList "-i", "`"$InputFile`"", "-af", "loudnorm=I=$TargetLUFS:TP=-1.5:LRA=7:measured_I=$measuredLoudness:measured_TP=0.0:measured_LRA=0.0:measured_thresh=-50.0:offset=0.0:linear=true", "-ar", $sampleRate, "-c:a", "libmp3lame", "-b:a", $bitRate, "-ac", "2", "-y", "`"$OutputFile`"" -Wait -PassThru -NoNewWindow
