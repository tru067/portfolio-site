# Working Audio Normalization Script for Windows
...
# Configuration
$TargetLUFS = "-14.0"
$LogFile = "normalization_log_working.txt"
...
function Normalize-File {
    ...
    # Extract integrated loudness from output - try multiple patterns
    if ($measureOutput -match 'I:\s*(-?\d+\.\d+)') {
        $measuredLoudness = $matches[1]
    } elseif ($measureOutput -match 'Integrated loudness:\s*(-?\d+\.\d+)') {
        $measuredLoudness = $matches[1]
    } elseif ($measureOutput -match 'Summary:\s*Integrated:\s*(-?\d+\.\d+)') {
        $measuredLoudness = $matches[1]
    }
    ...
    if ($measuredLoudness -ne "N/A") {
        ...
        # Apply conservative gain to prevent clipping (limit to +6dB max)
        if ($gainNeeded -gt 6.0) {
            $gainNeeded = 6.0
            Write-Host "  Gain limited to 6dB to prevent clipping"
        }
    } else {
        $gainNeeded = 0
    ...
    if (Test-Path $OutputFile) {
        ...
        if ($finalOutput -match 'I:\s*(-?\d+\.\d+)') {
            ...
            $finalLoudness = $matches[1]
        } elseif ($finalOutput -match 'Summary:\s*Integrated:\s*(-?\d+\.\d+)') {
            $finalLoudness = $matches[1]
        }
...
Write-Host "Volume adjustment applied (with clipping prevention)"
